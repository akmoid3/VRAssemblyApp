...
public unsafe class CoACD : MonoBehaviour
{
    ...
    static extern MeshArrayInterface Run(ref MeshInterface mesh, double threshold, int max_convex_hull,
        int preprocess_mode, int prep_resolution, int sample_resolution,
    ...
    public unsafe struct Parameters
    {
        ...
        [Range(0.01f, 1f)] [Tooltip("concavity threshold for terminating the decomposition")]
        ...
        [Range(20, 100)] [Tooltip("resolution for manifold preprocess")]
        public int preprocessResolution;

        [Range(1000, 10000)] [Tooltip("sampling resolution for Hausdorff distance calculation")]
        public int sampleResolution;

        [Range(10, 40)] [Tooltip("max number of child nodes in MCTS")]
        public int mctsNodes;

        [Range(60, 2000)] [Tooltip("number of search iterations in MCTS")]
        public int mctsIteration;

        [Range(2, 7)] [Tooltip("max search depth in MCTS")]
    ...
    [SerializeField] CoACDColliderData _colliderData;
    [SerializeField] List<MeshCollider> _colliders = new List<MeshCollider>();
#if UNITY_EDITOR
...
#endif
    public struct ComputedMeshData
    {
        public Vector3[] vertices;
        public int[] triangles;
    }

    public List<Mesh> CreateMeshesFromData(List<ComputedMeshData> computedData)
    {
        var meshes = new List<Mesh>();
        foreach (var data in computedData)
        {
            Mesh rmesh = new Mesh();
            rmesh.SetVertices(data.vertices);
            rmesh.SetTriangles(data.triangles, 0);
            meshes.Add(rmesh);
        }
    ...
    public unsafe List<ComputedMeshData> RunACD_ComputeData(
        Mesh mesh,
        Vector3[] vertices,
        int[] triangles,
        int vertexCount,
    ...
    {
        // Verifica subito se è già stata richiesta la cancellazione
        token.ThrowIfCancellationRequested();

        var unityV = vertices;
        var unityF = triangles;
        var v = new double[vertexCount * 3];

        for (var i = 0; i < vertexCount; i++)
        {
            // Controllo periodico per la cancellazione
        ...
        List<ComputedMeshData> computedMeshes = new List<ComputedMeshData>();
        fixed (double* vptr = v)
        {
            fixed (int* fptr = unityF)
            {
                var mi = new MeshInterface()
                {
                    vertices_ptr = vptr,
                    vertices_count = (ulong)vertexCount,
                    triangles_ptr = fptr,
                    triangles_count = (ulong)(unityF.LongLength / 3)
                };

                // Verifica la cancellazione prima della chiamata nativa
                if (token.IsCancellationRequested)
                    token.ThrowIfCancellationRequested();

                using var res = Run(ref mi, parameters.threshold, parameters.maxConvexHull,
                    (int)parameters.preprocessMode,
                    parameters.preprocessResolution, parameters.sampleResolution,
                    parameters.mctsNodes, parameters.mctsIteration, parameters.mctsMaxDepth,
                    parameters.pca, parameters.merge, parameters.seed);
                ...
                for (ulong i = 0; i < res.meshes_count; i++)
                {
                    ...
                    var meshData = new ComputedMeshData();
                    meshData.vertices = new Vector3[res.meshes_ptr[i].vertices_count];
                    for (ulong j = 0; j < res.meshes_ptr[i].vertices_count; j++)
                    {
                        if (token.IsCancellationRequested)
                            token.ThrowIfCancellationRequested();

                        meshData.vertices[j] = new Vector3(
                    ...
                    meshData.triangles = new int[res.meshes_ptr[i].triangles_count * 3];
                    for (ulong j = 0; j < res.meshes_ptr[i].triangles_count * 3; j++)
                    {
                        if (token.IsCancellationRequested)
                            token.ThrowIfCancellationRequested();

                        meshData.triangles[j] = res.meshes_ptr[i].triangles_ptr[j];
                    }

                    computedMeshes.Add(meshData);
                }
            }
        }

        return computedMeshes;
    }
    ...
    public List<Mesh> RunACD(Mesh mesh)
    {
        ...
        fixed (double* vptr = v)
        {
            fixed (int* fptr = unityF)
            {
                var mi = new MeshInterface()
                {
                    vertices_ptr = vptr, vertices_count = (ulong)mesh.vertexCount, triangles_ptr = fptr,
                    triangles_count = (ulong)(unityF.LongLength / 3)
                };
                using var res = Run(ref mi, parameters.threshold, parameters.maxConvexHull,
                    (int)parameters.preprocessMode, parameters.preprocessResolution, parameters.sampleResolution,
                    parameters.mctsNodes, parameters.mctsIteration, parameters.mctsMaxDepth, parameters.pca,
                    parameters.merge, parameters.seed);
                var meshes = new List<Mesh>();
                for (ulong i = 0; i < res.meshes_count; i++)
                {
                    var rmesh = new Mesh();
                    ...
                    for (ulong j = 0; j < res.meshes_ptr[i].vertices_count; j++)
                    {
                        verts[j] = new Vector3((float)res.meshes_ptr[i].vertices_ptr[j * 3 + 0],
                            (float)res.meshes_ptr[i].vertices_ptr[j * 3 + 1],
                            (float)res.meshes_ptr[i].vertices_ptr[j * 3 + 2]);
                    }
    ...
    void CalculateColliders()
    {
        ...
        var transformsToCalc = new List<Matrix4x4>();
        {
            ...
            else
            {
                EditorUtility.DisplayDialog("Error",
                    $"The object you are trying to calculate colliders for did not contain any MeshFilter or SkinnedMeshRenderer.",
                    "Ok");
                return;
            }
        ...
        var decomposedMeshes = new List<Mesh>();
        {
            ...
            {
                EditorUtility.ClearProgressBar();
                EditorUtility.DisplayProgressBar("Calculating Colliders",
                    $"Processing mesh... ({c++}/{meshesToDecompose.Count}) (this can take a while)",
        ...
        if (_colliderData == null)
        {
            ...
            _colliderData =
                CoACDColliderData.CreateAsset(path, parameters, decomposedMeshes.ToArray(), originalMeshes.ToArray());
        }
        else
        {
            _colliderData.UpdateAsset(parameters, decomposedMeshes.ToArray(), originalMeshes.ToArray());
        }
    ...
    void CalculateCollidersForHierarchy()
    {
        ...
        var transformsToCalc = new List<Matrix4x4>();
        {
            ...
            {
                EditorUtility.ClearProgressBar();
                EditorUtility.DisplayProgressBar("Calculating Colliders for Hierarchy",
                    $"Discovering mesh... ({c++}/{childComponents.Count})",
    ...
    void ValidateColliders(Transform baseTransform)
    {
        if (_colliderData != null)
        {
            ...
            if (_colliders.Count != _colliderData.computedMeshes.Length)
            {
                if (_colliders.Count > _colliderData.computedMeshes.Length)
                {
                    for (var i = _colliders.Count - 1; i >= _colliderData.computedMeshes.Length; i--)
                    {
                ...
                {
                    for (var i = _colliders.Count; i < _colliderData.computedMeshes.Length; i++)
                    {
                        _colliders.Add(baseTransform.gameObject.AddComponent<MeshCollider>());
                    }
                }
            }
...
public class CoACDColliderData : ScriptableObject
{
    ...
    public static CoACDColliderData CreateAsset(string path, CoACD.Parameters parameters, Mesh[] meshes,
        Mesh[] baseMeshes)
    {
